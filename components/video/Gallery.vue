    ...
    <div class="flex flex-col gap-8 mt-10">
        <div class="w-full text-center flex flex-col gap-4">
            ...
            <p class="w-[40%] mx-auto">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Nam maxime nostrum
                cupiditate velit optio obcaecati cumque, consectetur aliquam explicabo eaque!</p>
        </div>

        <div class="card flex flex-col items-center w-full">
            <Galleria v-model:activeIndex="activeIndex" v-model:visible="displayCustom" :value="videos"
                ...
                <template #item="slotProps">
                    <div class="relative w-full">
                        ...
                        <iframe :src="slotProps.item.videoUrl" frameborder="0" allowfullscreen
                            class="lg:w-[700px] w-[400px] lg:h-[500px] h-[350px]   rounded-lg"></iframe>
                    </div>
                </template>
                <template #thumbnail="slotProps">
                    <img :src="slotProps.item.thumbnailImageSrc" :alt="slotProps.item.alt" class="block rounded-lg" />
                </template>
            </Galleria>
            ...
            <div v-if="videos" class="grid grid-cols-3 gap-8 w-full mt-8">
                <div v-for="(video, index) of videos" :key="index" class="w-full relative rounded-lg">
                    ...
                    <img :src="video.thumbnailImageSrc" :alt="video.alt" class="w-full cursor-pointer rounded-lg"
...
import { VideoService } from './video.js'; // Updated to VideoService

onMounted(() => {
    VideoService.getVideos().then((data) => (videos.value = data));
});
